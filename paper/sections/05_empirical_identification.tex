...
public-good provision, by income quintile and consumption category.}
\label{tab:micro-main}
\begin{tabular}{@{}crrrr@{}}
\toprule
Quintile & {$\beta^q$ complement} & {(SE)} & {$\beta^q$ substitute} & {(SE)} \\
\midrule
1 (poorest) & +0.187 & 0.042 & -0.032 & 0.045 \\
2           & +0.142 & 0.039 & -0.048 & 0.040 \\
3           & +0.068 & 0.034 & -0.097 & 0.038 \\
4           & +0.024 & 0.032 & -0.124 & 0.041 \\
5 (richest) & -0.021 & 0.035 & -0.158 & 0.041 \\
\bottomrule
\end{tabular}
\end{table}
...
The joint posterior distribution over $(\theta^{H},\theta^{S})$ shows that
the two parameters are negatively correlated across draws -- plausibly
because the micro elasticities are identified relative to a common time
trend. The 68\% credible region excludes both the separable case
($\theta^{H}=\theta^{S}=0$) and symmetric configurations
($\theta^{H}=\theta^{S}$).
